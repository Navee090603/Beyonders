            ...
            foreach (var p in paymentsEntities)
            {
                ...
                if (emiCalc != null)
                {
                    ...
                    foreach (var inst in instEntities)
                    {
                        ...
                        {
                            InstallmentId = installmentIdObj ?? 0,   // fallback to 0 if null
                            DueDate = dueDateObj ?? DateTime.MinValue, // fallback to MinValue if null
                            Amount = amountObj ?? 0m,   // fallback to 0 if null
